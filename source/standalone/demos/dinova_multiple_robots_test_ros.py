...
def main():
    ...
    joint_state_pub = rospy.Publisher("/joint_state", JointState, queue_size=10)
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            try:
                ...
                img_msg = bridge.cv2_to_imgmsg(rgb_image, encoding='bgr8')
                
                # Publish the image
                image_pub.publish(img_msg)
                rospy.loginfo("Image published successfully.")
            except Exception as e:
                rospy.logerr("Error converting or publishing image: %s", str(e))
